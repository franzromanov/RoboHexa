...
//leg length
float leg_l(float t,float T, float inp_ax,float inp_a0,float input_az,float inp_z0){
	return sqrt(pow(inp_ax-func_x0(t,T,inp_ax,inp_a0),2)+pow(func_z1( T, t, inp_az)-inp_z0,2));

}
float _d=leg_l(t,T,ax,a0,az,z0);
float _a1,_a2;


//angle

float func_angle0(float t,float T, float inp_ax,inp_Ay){
	return atan(func_y1(t,T,inp_Ay)/inp_ax);
}

float func_angle1(float t,float T,float d float a1, float a2,float inp_ax,float inp_a0){

	return atan(func_z1(t,T,inp_az)/(inp_ax-func_x0(t,T,inp_ax,inp_a0))) + acos((pow(a2,2)-pow(a1,2)+pow(d,2))/(2*a1*a2));
}

float func_angle2(float t,float a1,float a2){
...
theta1=deg(func_angle1(_t,_T,_d,_a1,_a2,ax,a0));
theta2=deg(func_angle2(_t,_T,_a1,_a2));
